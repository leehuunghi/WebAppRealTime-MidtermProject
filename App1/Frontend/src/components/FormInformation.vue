<template>
  <div>
    <div
      ...
      style="height: 100% !important; display: none; width: 100% !important; background-color: transparent !important;"
    >
      <div
        class="modal"
        role="dialog"
        style="display: block; padding-top: 200px; background-color: rgba(0,0,0,0.5);"
      >
        <div class="modal-dialog" role="document">
          ...
          >
            ...
            <div style="text-align: right">
              <button id="tryAgain" type="button" class="mdBtn" v-on="TryAgain()">Nhập lại</button>
            </div>
          </div>
        </div>
      </div>
    ...
    >
      ...
      >
        <div class="modal-dialog" role="document">
          <div
            class="modal-content"
            style="padding: 20px; border: none !important; border-radius: 10px; border: 5px solid #0FA9D6;"
          >
            <div>
            ...
            <div style="text-align: right">
              <button id="close" type="button" class="mdBtn" v-on:click="Close()">Đóng</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="coverBg">
      <div
        id="logoutBtn"
        style="position: fixed; width: 100%; text-align: right; z-index: 99;"
        v-on:click="LogOut()"
      >
        <button class="logoutBtn">ĐĂNG XUẤT
          <Table></Table>
        </button>
      </div>
      <img
        id="cover"
        src="/static/pics/cover-receiver.png"
        style="width: 100%; height: auto; opacity: 1; margin-bottom: 0px;"
      >
    </div>
    <div class="container">
      <div class="content blueText center" style="margin-top: -60px;">
        ...
        <form
          style="margin: 20px 50px 0 150px;"
          action
          method="POST"
          v-on:submit.prevent="sendinfo"
        >
          <div class="row">
            <div class="col-md-5">
            ...
            <div class="col-md-1">
              <button type="submit" class="submitBtn" style="margin-top: 122px; margin-left: -20px">
  ...
  methods: {
    sendinfo() {
      ...
      if ($("#address").val() == "") {
        ...
        setTimeout(function() {
          $("#address").removeClass("errorShake");
        }, 500);
        flag = false;
      }
      if ($("#phone").val() == "") {
      ...
      if (flag == true) {
        axios
          .post(
            `http://${IPGlobal.IP}:3000/api/bookingBike/book`,
            this.formdata,
            {
              headers: {
